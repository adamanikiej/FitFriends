...
const workoutsDB = require('../data/workouts');
...
router.post('/create', async (req, res) => {
  ...
  if (errors.length > 0) {
    ...
    try {
      let createdWorkout = await workoutsDB.createWorkout(req.headers.authorization, req.body);
      if (createdWorkout) {
...
router.get('/list', async (req, res) => {
  ...
  if (!req.headers.authorization) {
    ...
    try {
      let workoutList = await workoutsDB.getWorkouts(req.headers.authorization);
