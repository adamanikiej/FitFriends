    ...
    const fetchData = async () => {
      try {
        const [workoutData, logData] = await Promise.all([
          ...
          // add other axios call once workout log is done
        ]);

        setWorkoutList(workoutData.data);
        setLoading(false);
        // add other state set for workout log
...
function CreateWorkout({ navigation }) {
  ...
  const exerciseList = ['Ab Crunch Machine', 'Ab Roller', 'Bear Crawls', 'Flutter Kicks', ' Bicycles', 'Plank', 
    ...
    'Cable Tricep Pull Down', 'Machine Tricep Dips', 'Cable Overhead Tricep Extensions', 'Barbell Close-Grip Press', 'Diamond Push-ups']
                      ...
                      {exercise.sets.map((set, j) => {
                        return (
                          <View style={{marginTop: j !== 0 ? 10 : 4}} key={j}>
                            <View style={{display: 'flex', flexDirection: 'row', justifyContent: 'space-between', width: '105%'}}>
                              <View style={{display: 'flex', flexDirection: 'row', alignItems: 'center', marginLeft: -22, marginRight: 40}}>
...
export function Workout({ navigation }) {
  return (
    ...
    >
      <Stack.Screen name="ViewWorkouts" component={ViewWorkouts} options={{title: 'Fit Friends'}} initialParams={{ newWorkout: {}}}/>
